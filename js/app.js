...
function updateNetState(){
  $("#netState").textContent = navigator.onLine ? "online" : "offline";
}
...
function findHeaderRow(allRows){
  ...
  for(let i=0;i<limit;i++){
    const sc = scoreHeaderRow(allRows[i]);
...
function fillMapSelectors(hs){
  ...
  fill($("#mapPrice"));
  const g = guessMap(hs);
  $("#mapPart").value = g.part;
  $("#mapEan").value  = g.ean;
  $("#mapDesc").value = g.desc;
  $("#mapPrice").value= g.price;
}
...
function uniqByKey(list){
  ...
  for(const r of list){
    const key =
      (r.partNumber && r.partNumber.trim().toLowerCase()) ||
      (r.ean && r.ean.trim().toLowerCase()) ||
      ((r.partNumber||"")+"|"+(r.ean||"")+"|"+(r.description||"")).toLowerCase();
...
function filtered(all){
  ...
  outer: for(const r of all){
    ...
    tmp.push(r);
    if(tmp.length >= 5000) break;
  }
  const unique = uniqByKey(tmp);
...
function doSearch(){
  ...
  $("#searchInfo").textContent = list.length>=1000 ? " (mostrando primeros 1000 tras deduplicar)" : "";
  renderTable(list);
}
...
$$("th[data-k]").forEach(th => th.addEventListener("click", () => {
  ...
  if(window._sortKey === key) window._sortDir *= -1;
  else { window._sortKey = key; window._sortDir = 1; }
  const list = filtered(DATA).sort((a,b)=>{
    const av = a[key] ?? "";
    const bv = b[key] ?? "";
...
async function handleFile(file){
  ...
  if(name.endsWith(".csv")){
    ...
    if(!allRows.length){ showToast("CSV vacío"); return; }
    const headerIdx = findHeaderRow(allRows);
    headers = allRows[headerIdx].map(h => (h??"").toString().trim());
    ...
    const allRows = window.XLSX.utils.sheet_to_json(ws,{header:1,raw:false,defval:""});
    if(!allRows.length){ showToast("Excel sin datos"); return; }
    const headerIdx = findHeaderRow(allRows);
    headers = allRows[headerIdx].map(h => (h??"").toString().trim());
    rows = allRows.slice(headerIdx+1);
    fillMapSelectors(headers);
...
$("#saveBtn").addEventListener("click", async ()=>{
  ...
  const items = [];
  const seen = new Set(); // deduplicación ya al guardar
  for(const r of rows){
    const rawPart = (r[idx.part]  ?? "").toString().trim();
    const rawEan  = (r[idx.ean]   ?? "").toString().trim();
    const rawDesc = (r[idx.desc]  ?? "").toString().trim();
    const rawPrice= (r[idx.price] ?? "").toString().trim();
    const key =
      (rawPart && rawPart.toLowerCase()) ||
      (rawEan  && rawEan.toLowerCase())  ||
      (rawPart+"|"+rawEan+"|"+rawDesc).toLowerCase();
    if(seen.has(key)) continue;
    seen.add(key);
    const it = {
      partNumber: rawPart,
      ...
      description: rawDesc,
      priceStr: rawPrice,
      price: parsePriceSmart(rawPrice)
    };
  ...
  $("#mapBox").classList.add("hidden");
  $("#q").focus();
  showToast(`Guardado ${items.length} filas (sin duplicados) en este dispositivo.`);
});
